...
new_dataset_dir = "FET_20_40_TBR_348_homogen_v1_646432_test"


new_dataset_path = os.path.join(base_dataset_path.rsplit(os.path.sep, 1)[0], new_dataset_dir)
if not os.path.isdir(new_dataset_path):
    os.makedirs(new_dataset_path)

new_tumor_ROIs_path = os.path.join(new_dataset_path, "tumor_ROIs")
if not os.path.isdir(new_tumor_ROIs_path):
    os.makedirs(new_tumor_ROIs_path)

new_brain_ROIs_path = os.path.join(new_dataset_path, "brain_ROIs")
if not os.path.isdir(new_brain_ROIs_path):
    os.makedirs(new_brain_ROIs_path)

new_GT_masks_path = os.path.join(new_dataset_path, "GT_masks")
if not os.path.isdir(new_GT_masks_path):
    os.makedirs(new_GT_masks_path)

os.chdir(base_dataset_path)
print(f"Current working path: {os.getcwd()}")
print(f"Saving new dataset to: {new_dataset_path}")

images_list = glob.glob("*.nii")
...
for i in tqdm(range(len(images_list))):
    ...
    sitk.WriteImage(cropped_padded_img, os.path.join(new_dataset_path, images_list[i]))
